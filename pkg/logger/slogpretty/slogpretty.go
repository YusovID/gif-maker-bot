...
type PrettyHandler struct {
	slog.Handler
	l     *log.Logger // Используется стандартный `log` для вывода, чтобы избежать рекурсии.
...
func SetupLogger(env string) *slog.Logger {
	...
	logAdapter := slog.NewLogLogger(handler, logLevel)
	log.SetOutput(logAdapter.Writer())
	log.SetPrefix("")
	log.SetFlags(0)
